...
func TestWriteReadAgentNames(t *testing.T) {
	path := filepath.Join(t.TempDir(), "skills.lock.json")
	now := time.Now().UTC().Truncate(time.Second)

	lf := &File{Version: fileVersion, Installs: []Install{
		{
			Scope:               "local",
			Repo:                "o/r",
			Skill:               "sk",
	...
	if len(got.Installs[0].InstalledAgentNames) != 1 || got.Installs[0].InstalledAgentNames[0] != "claude" {
		t.Errorf("InstalledAgentNames = %v, want [claude]", got.Installs[0].InstalledAgentNames)
	}
}

func TestReadOldLockWithoutAgentNames(t *testing.T) {
	path := filepath.Join(t.TempDir(), "skills.lock.json")
	// Write a lock file without the InstalledAgentNames field.
	data := `{"version":1,"installs":[{"scope":"local","repo":"o/r","skill":"sk","installed_targets":["/x"]}]}`
	...
	if got.Installs[0].InstalledAgentNames != nil {
		t.Errorf("expected nil InstalledAgentNames for old lock, got %v", got.Installs[0].InstalledAgentNames)
	}
}
...
func TestWriteOverwrite(t *testing.T) {
	...
	lf1 := &File{Version: fileVersion, Installs: []Install{
		{Scope: "local", Repo: "a/b", Skill: "sk1", InstalledAt: now, UpdatedAt: now},
	}}
	if err := Write(path, lf1); err != nil {
		t.Fatal(err)
	}

	lf2 := &File{Version: fileVersion, Installs: []Install{
		{Scope: "local", Repo: "x/y", Skill: "sk2", InstalledAt: now, UpdatedAt: now},
	}}
	if err := Write(path, lf2); err != nil {
		t.Fatal(err)
	}
	...
	if got.Installs[0].Repo != "x/y" {
		t.Errorf("Repo = %q, want %q", got.Installs[0].Repo, "x/y")
	}
}
