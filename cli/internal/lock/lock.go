...
type Install struct {
	...
	InstalledAgentNames []string  `json:"installed_agent_names,omitempty"`
...
func Write(path string, lf *File) error {
	...
	_ = os.Remove(path) // ignore if not exists; needed for Windows where Rename fails if dest exists
