...
// Table renders a bordered table with a styled header.
func Table(headers []string, rows [][]string) {
	...
	r := outRenderer()
	headerStyle := r.NewStyle().Bold(true).Foreground(lipgloss.Color("245")).PaddingLeft(1).PaddingRight(1)
	cellStyle := r.NewStyle().PaddingLeft(1).PaddingRight(1)
	...
	t := table.New().
		...
		BorderRow(true).
		BorderStyle(r.NewStyle().Foreground(lipgloss.Color("238"))).
...
// termWidth returns the terminal width, or 0 if not a TTY.
