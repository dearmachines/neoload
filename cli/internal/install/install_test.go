...
func TestInstallSingleTarget(t *testing.T) {
	...
	paths, skipped, count, err := Install(skillFS, tgts, "myskill", false)
	if err != nil {
...
func TestInstallExistingNoForce(t *testing.T) {
	...
	installed, skipped, _, err := Install(skillFS, tgts, "myskill", false)
	if err != nil {
	...
	if skipped[0] != destDir {
		t.Errorf("skipped = %q, want %q", skipped[0], destDir)
	}
}
...
func TestInstallExistingWithForce(t *testing.T) {
	...
	_, _, _, err := Install(skillFS, tgts, "myskill", true)
	if err != nil {
...
func TestInstallNewAgentSkipsExisting(t *testing.T) {
	dir := t.TempDir()
	skillFS := makeSkillFS(map[string]string{"SKILL.md": "# Test"})

	// claude already has the skill, opencode is new.
	claudeDir := filepath.Join(dir, ".claude/skills")
	opencodeDir := filepath.Join(dir, ".opencode/skills")
	os.MkdirAll(filepath.Join(claudeDir, "myskill"), 0755)

	tgts := []targets.Target{
		{AgentName: "claude", SkillsDir: claudeDir},
		{AgentName: "opencode", SkillsDir: opencodeDir},
	}

	installed, skipped, count, err := Install(skillFS, tgts, "myskill", false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(installed) != 1 {
	...
	if installed[0] != filepath.Join(opencodeDir, "myskill") {
		t.Errorf("installed = %q, want opencode path", installed[0])
	}
	if len(skipped) != 1 {
		t.Fatalf("expected 1 skipped, got %d", len(skipped))
	}
	if skipped[0] != filepath.Join(claudeDir, "myskill") {
		t.Errorf("skipped = %q, want claude path", skipped[0])
	}
	if count != 1 {
...
func TestInstallNestedFiles(t *testing.T) {
	...
	_, _, count, err := Install(skillFS, tgts, "myskill", false)
	if err != nil {
		t.Fatalf("Install: %v", err)
	}
	if count != 3 {
